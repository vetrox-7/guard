...
client.on('ready', async () => {
 ...
 client.user.setActivity(`Hyperion Guard Bot `, { type:'PLAYING' })
  
  console.log("Hyperion Guard Bot")
});
...
client.login("ODc5MzA4NzAxMzM5NjE5NDA4.YSN2Cw.OfZqYGGeQmdmB6nqGvoDohTQm2Q")
...
let kanal = await db.fetch(`antiraidK_${member.guild.id}`)== "anti-raid-aç"
  ...
  if (member.user.bot === true) {
    ...
    let darknesguardv2 = new Discord.MessageEmbed()
      ...
      .setDescription(`**${member.user.tag}** (${member.id}) adlı bota bir yetkili izin verdi eğer kaldırmak istiyorsanız **vc!bot-izni kaldır <botid>**.`);
    darknesyt.send(darknesguardv2);
     } else {
       let izinverilmemişbot = new Discord.MessageEmbed()
      .setColor("RANDOM")
      .setThumbnail(member.user.avatarURL())
      .setDescription("**" + member.user.tag +"**" + " (" + member.id+ ") " + "adlı bot sunucuya eklendi ve banladım eğer izin vermek istiyorsanız **" + "vc!bot-izni ver <botid>**")
...
client.on("ready", async () => {
  let botVoiceChannel = client.channels.cache.get("857626268572057640");
  console.log("Ses Kanalına Girdi");
    if (botVoiceChannel)
      botVoiceChannel
        .join()
        .catch(err => console.error("Ses Kanalı Baarılı")); 
  });
          ...
          try {
            if (!msg.member.permissions.has("BAN_MEMBERS")) {
                  msg.delete();
                      ...
                      return msg.reply('Küfür Yasak.').then(nordx => nordx.delete({timeout: 5000}))
            }              
          } catch(err) {
            console.log(err);
          }
 ...
 if (veri) {
        ...
        if (reklam.some(word => msg.content.includes(word))) {
          try {
            if (!msg.member.permissions.has("BAN_MEMBERS")) {
                  msg.delete();
                    return msg.reply('Reklam Yasak.').then(nordx => nordx.delete({timeout: 5000}))                          
...
let hereengelle = await db.fetch(`hereengel_${msg.guild.id}`)
 if (hereengelle == 'acik') {
  ...
  if (here.some(word => msg.content.toLowerCase().includes(word)) ) {
    if (!msg.member.permissions.has("ADMINISTRATOR")) {
      msg.delete()
       return msg.reply('Everyone ve Here Etiketlemek Yasak.').then(nordx => nordx.delete({timeout: 5000}))
        }
    }
...
//Modlog Son
client.on("ready", () => {
  client.channels.cache.get("873348154857246763").join();
})